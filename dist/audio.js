...
var Media = exports.Media = function () {
    function Media() {
        ...
        var that = this;
        var oldUserMedia = function oldUserMedia(constraints) {
            var getUserMedia = navigator.getUserMedia || navigator.webketGetUserMedia || navigator.mozGetUserMedia;
...
exports.audiojs = undefined;

var _media = __webpack_require__(124);

var _player = __webpack_require__(330);
...
var _incontext = __webpack_require__(331);

var _outcontext = __webpack_require__(332);

var _config = __webpack_require__(333);

var _encoder = __webpack_require__(334);

var _ws = __webpack_require__(335);

var _blob = __webpack_require__(336);

var _as3context = __webpack_require__(337);

var _audio = __webpack_require__(338);
...
var Player = exports.Player = function () {
    function Player(config) {
        ...
        this._config = {};
        Object.assign(this._config, _config.defaultConfig, config ? config : _config.defaultConfig);
        var that = this;
        var outcontext = new _outcontext.OutAudioContext(that._config);
        var audio = new _audio.Audio();
        this._incontext = null;
        this._as3context = null;

        if (!outcontext.isSupport()) {
            this._as3context = new _as3context.As3Context(that._config);
        }
        this._ws = new _ws.WS(this._config.url);
        this._ws.on('received', function (dataBuffer) {
            if (outcontext.isSupport()) {
                outcontext.start();
                outcontext.pushAudioData(dataBuffer);
            } else {
                var wavblob = _blob.BlobData.wav(dataBuffer);
                audio.play(wavblob);
            }
        });
    }
    ...
    _createClass(Player, [{
        ...
        value: function ready() {
            ...
            if (media._isSupported) {
                media.promiseStream().then(function (medisStream) {
                    that._incontext = new _incontext.InContext(that._config, medisStream);
                }).catch(function (e) {
                    console.log(e);
                });
            } else {
        ...
        value: function send() {
            if (this._incontext) {
                var dataBuffer = this._incontext.get();
                var wavDataBuffer = _encoder.Encoder.wav(dataBuffer, this._config.outputSampleRate, this._config.outputSampleBits, this._config.numberChannels);
                this._ws.send(wavDataBuffer);
                this._incontext.stop();
                this._incontext.clear();
            } else if (this._as3context) {
                var _dataBuffer = this._as3context.get();
                var _wavDataBuffer = _encoder.Encoder.wav(_dataBuffer, this._config.outputSampleRate, this._config.outputSampleBits, this._config.numberChannels);
                this._ws.send(_wavDataBuffer);
        ...
        value: function getbuffer(cb) {
            if (this._incontext && typeof cb === "function") {
                ...
                var _dataBuffer2 = this._as3context.get();
                this._as3context.stop();
                this._as3context.clear();
                cb(_dataBuffer2);
            }
        ...
        key: "exportwav",
        value: function exportwav(cb) {
            if (this._incontext && typeof cb === "function") {
                var dataBuffer = this._incontext.get();
                var wavDataBuffer = _encoder.Encoder.wav(dataBuffer, this._config.outputSampleRate, this._config.outputSampleBits, this._config.numberChannels);
                var wavblob = _blob.BlobData.wav(wavDataBuffer);
                this._incontext.stop();
                this._incontext.clear();
                cb(wavblob);
            } else if (this._as3context) {
                var _dataBuffer3 = this._as3context.get();
                var _wavDataBuffer2 = _encoder.Encoder.wav(_dataBuffer3, this._config.outputSampleRate, this._config.outputSampleBits, this._config.numberChannels);
                var _wavblob = _blob.BlobData.wav(_wavDataBuffer2);
                this._as3context.stop();
                this._as3context.clear();
                cb(_wavblob);
            }
        ...
        key: "forcedownload",
        value: function forcedownload(filename) {
...
exports.InContext = undefined;

var _createClass = function () { function defineProperties(target, props) { for (var i = 0; i < props.length; i++) { var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, descriptor.key, descriptor); } } return function (Constructor, protoProps, staticProps) { if (protoProps) defineProperties(Constructor.prototype, protoProps); if (staticProps) defineProperties(Constructor, staticProps); return Constructor; }; }();

var _context = __webpack_require__(125);
...
var InContext = exports.InContext = function (_Context) {
    _inherits(InContext, _Context);
     ...
     */
    function InContext(config, mediaStream) {
        _classCallCheck(this, InContext);

        var _this = _possibleConstructorReturn(this, (InContext.__proto__ || Object.getPrototypeOf(InContext)).call(this, config));
        ...
        _this._scriptProcessorNode = _this._context.createScriptProcessor(_this._config.bufferSize, _this._config.numberChannels, _this._config.numberChannels);
        _this._mediaStreamAudioSourceNode = _this._context.createMediaStreamSource(mediaStream);

        _this._mediaStreamAudioSourceNode.connect(_this._scriptProcessorNode);
        _this._scriptProcessorNode.connect(_this._context.destination);

        var that = _this;
        _this._scriptProcessorNode.onaudioprocess = function (audioProcessEvent) {
    ...
    _createClass(InContext, [{
        ...
        key: "get",
        value: function get() {
            var inputSampleRate = this._context.sampleRate; //输入采样率
            var outSampleRate = this._config.outputSampleRate; //输出采样率
            //合并数组中的数组为1个连续数组
            var mergeBuffers = function mergeBuffers(buffer, length) {
            ...
            if (this._config.numberChannels === 1) {
                ...
                var dataL = mergeBuffers(this._LBuffer, this._lsize);
                var dataR = mergeBuffers(this._RBuffer, this._rsize);
                data = interleave(dataL, dataR);
            }
            //压缩
            var compression = parseInt(inputSampleRate / outSampleRate);
...
exports.OutAudioContext = undefined;
...
var OutAudioContext = exports.OutAudioContext = function (_Context) {
    _inherits(OutAudioContext, _Context);

    function OutAudioContext(config) {
        _classCallCheck(this, OutAudioContext);

        return _possibleConstructorReturn(this, (OutAudioContext.__proto__ || Object.getPrototypeOf(OutAudioContext)).call(this, config));
    }

    _createClass(OutAudioContext, [{
        key: "start",
        value: function start() {
        ...
        key: "pushAudioData",
        value: function pushAudioData(audioData) {
            var _this2 = this;

            this._context.decodeAudioData(audioData).then(function (audioBuffer) {
                _this2._audioBufferSourceNode.buffer = audioBuffer;
            }).catch(function (e) {
                throw e;
            });
        }
        ...
        key: "pushDataArray",
        value: function pushDataArray(arrayBuffer) {
            var audioBuffer = this._context.createBuffer(this._config.numberChannels, this._context.sampleRate * 5, this._context.sampleRate);
            for (var index = 0; index < this._config.numberChannels.length; index++) {
                var tempBuffer = audioBuffer.getChannelData(index); //TODO
...
var defaultConfig = exports.defaultConfig = {
  ...
  outputSampleRate: 44100 / 6, //输出采样率 7350
...
var Encoder = exports.Encoder = function () {
    ...
    _createClass(Encoder, null, [{
        key: 'wav',
        value: function wav(bytes, sampleRate, sampleBits, outChannelsNumber) {
            var dataLength = bytes.length * (sampleBits / 8);
            var buffer = new ArrayBuffer(44 + dataLength);
            var data = new DataView(buffer);

            var channelCount = outChannelsNumber;
            var offset = 0;
 ...
 * limitations under the License.
 */
var BlobData = exports.BlobData = function () {
    function BlobData() {
        _classCallCheck(this, BlobData);
    }

    _createClass(BlobData, null, [{
        key: 'wav',
        value: function wav(data) {
 ...
 * limitations under the License.
 */
var As3Context = exports.As3Context = function () {
    function As3Context(config) {
        _classCallCheck(this, As3Context);
        ...
        var swfpath = config.swfPath ? config.swfPath : './audio.swf';
        ...
        if (swfobject) {
            ...
            swfobject.embedSWF(swfpath, domid, ele.clientWidth, ele.clientHeight, '25.0.0', './expressInstall.swf', flashvars, params, attributes);
